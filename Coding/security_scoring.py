...
COMPULSORY_PARAMETERS = {
    ...
    "AutoPlay Disabled": lambda: "YES" if "Disabled" in get_autoplay_status() else "NO",
    "Remote Desktop Disabled (RDP)": lambda: "YES" if "Disabled" in get_rdp_status() else "NO",
    ...
    "Passwords Not Saved": lambda: "YES" if "Not Saved" in check_saved_passwords() else "NO",
    "Unwanted Software Not Installed": lambda: "YES" if len(detect_unwanted_software() or []) == 0 else "NO",
    "Wi-Fi Disabled": lambda: "YES" if is_wifi_disabled() else "NO",
    "BIOS and Login Passwords Set": lambda: "YES" if passwords_set() else "NO",
}
...
def get_geolocation_status_systeminfo():
    ...
    return "Disabled" in info.get("Geolocation Service Status", "Unknown")
...
def passwords_set():
    bios_status = get_bios_password_status()
    login_status = get_login_password_status()
    return ("Set" in bios_status) and ("Set" in login_status)
