...
def removable_devices_page():
    ...
    def toggle_device(device_type):
        if device_type == 'usb':
            current = removable_device_control.get_usb_status()
            msg = removable_device_control.set_usb_status(disable=not current)
        elif device_type == 'cd':
            current = removable_device_control.get_cd_status()
            msg = removable_device_control.set_cd_status(disable=not current)
        else:
            return

        messagebox.showinfo("Device Access", msg + "\n\n⚠️ A system restart is recommended for changes to take effect.")
        update_status()
